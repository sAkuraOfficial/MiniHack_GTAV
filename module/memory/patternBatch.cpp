...
patternBatch::patternBatch(MODULEENTRY32 module, memoryContral* memManager)
    : m_moduleBase((uintptr_t)module.hModule), m_moduleSize(module.modBaseSize), m_moduleBytes(m_moduleSize)
{
    memManager->readMem(m_moduleBase, &m_moduleBytes[0], m_moduleSize);
}
